...
class WajbaManageController extends Controller
{
    public function add_wajba(Request $request)
    {
        $rules = [
            ...
            'latitude' => 'required',
            'longitude' => 'required',
            'experience_image' => 'required|image|mimes:jpeg,bmp,png,jpg|max:5024',
            'gallery_files.*' => 'image|mimes:jpeg,bmp,png,jpg|max:5024',
        ];
        ...
        if ($request->hasFile('experience_image')) {
            ...
            $url_thumb = 'frontend/imgs/thumbnail/' . $fileName;
            ...
            $img_thumbnail =Image::make($url)->fit(150, 120, function ($constraint) {
                $constraint->upsize();
            });

            $img->save($url);
            $img_banner->save($url_banner);
            $img_thumbnail->save($url_thumb);
    ...
    public function edit_wajba(Request $request)
    {
        ...
        if ($request->hasFile('experience_image')) {
            $rules['experience_image'] = 'image|mimes:jpeg,bmp,png|max:5024';
        }
        if ($request->hasFile('gallery_files')) {
            $rules['gallery_files.*'] = 'image|mimes:jpeg,bmp,png|max:5024';
        }
        $validator = Validator::make($request->all(), $rules);
