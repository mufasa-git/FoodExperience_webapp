...
class HomeController extends Controller
{
    ...
    public function profile_edit(UploadRequest $request)
    {
        $user = Auth::user();
        $validation_array = [
            'firstName' => 'required',
            'lastName' => 'required',
            'photo' => 'image|mimes:jpeg,bmp,png,jpg|max:1024',
    ...
    public function create_hihome(Request $request)
    {
        $request->validate([
            ...
            'lat' => 'required',
            'lot' => 'required',
            'experience_image' => 'required|image|mimes:jpeg,bmp,png|max:10024',
            // 'banner_file' => 'required|image|mimes:jpeg,bmp,png|max:1024',
            'gallery_files.*' => 'image|mimes:jpeg,bmp,png|max:10024',
        ], [
    ...
    public function edit_hihome(Request $request)
    {
        ...
        if ($request->hasFile('experience_image')) {
            $rules['experience_image'] = 'image|mimes:jpeg,bmp,png|max:9024';
        }
        ...
        if ($request->hasFile('gallery_files')) {
            $rules['gallery_files.*'] = 'image|mimes:jpeg,bmp,png|max:9024';
        }
